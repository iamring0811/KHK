...
/* Private define ------------------------------------------------------------*/
//#define NAND_FLASH_START_ADDR     ((uint32_t)0x70000000) 		//BANK2
...
//  GPIO_InitStructure.GPIO_Pin =  GPIO_Pin_7;				//Bamk 2
//  GPIO_Init(GPIOD, &GPIO_InitStructure);					//Bank 2
  GPIO_InitStructure.GPIO_Pin =  GPIO_Pin_9;				//Bank 3
  GPIO_Init(GPIOG, &GPIO_InitStructure);					//Bank 3
...
//  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;
  ...
  /*-- FMC Configuration ------------------------------------------------------*/
  p.FMC_SetupTime = 4;		//0xf5;
  p.FMC_WaitSetupTime = 3;	//0xf3;
  p.FMC_HoldSetupTime = 3;	//0xf2;
  p.FMC_HiZSetupTime = 5;	//0xf5;
...
//  FMC_NANDInitStructure.FMC_Bank = FMC_Bank2_NAND;
  ...
  FMC_NANDInitStructure.FMC_TCLRSetupTime = 3;		//0xa1;
  FMC_NANDInitStructure.FMC_TARSetupTime = 3;		//0x15;
...
void FMC_NAND_Test(void)
{
	...
	printf0("\r Start Test1");
//  FMC_NANDDeInit(FMC_Bank2_NAND);
  	FMC_NANDDeInit(FMC_Bank3_NAND);
	printf0("\r Start Test2");
  	FMC_NAND_Init();
	printf0("\r Start Test3");

  /* ¼ì²âNAND Flash */
  	FMC_NAND_ReadID(&NAND_ID);
  	printf0("\r Nand Flash ID = %02X,%02X,%02X,%02X  ",NAND_ID.Maker_ID, NAND_ID.Device_ID,
  	...
  	{
//	 	printf0("\r Type = K9F1G08U0A");
	 	printf0("\r Type = S34ML01G100TFI00");
  	}else{
	 	printf0("\r Type = Unknow");
  	}
  	...
  	for (index = 0; index < NAND_PAGE_SIZE; index+=2 )
  	{
//		nandTxBuffer[index] = index;
		nandTxBuffer[index+0] = k>>8;
		nandTxBuffer[index+1] = k++;
  	}
