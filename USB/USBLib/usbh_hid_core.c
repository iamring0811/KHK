...
static USBH_Status USBH_HID_ClassRequest(USB_OTG_CORE_HANDLE *pdev , 
                                         void *phost)
{   
	//UsbPrint("\r\n (usbh_core.USBH_Process gState : HOST_CLASS_REQUEST) -->  " );
  ...
  {
  case HID_IDLE:
			//UsbPrint("\r\n USBH_HID_ClassRequest HID_IDLE" );		
  case HID_REQ_GET_HID_DESC:
    ...
    if (USBH_Get_HID_Descriptor (pdev, pphost, USB_HID_DESC_SIZE)== USBH_OK)
    {
      ...
      HID_Machine.ctl_state = HID_REQ_GET_REPORT_DESC;
			//UsbPrint("\r\n USBH_HID_ClassRequest HID_REQ_GET_HID_DESC -->HID_REQ_GET_REPORT_DESC" );
    }
  ...
  case HID_REQ_GET_REPORT_DESC:
    ...
    if (USBH_Get_HID_ReportDescriptor(pdev , pphost, HID_Desc.wItemLength) == USBH_OK)
    {
      HID_Machine.ctl_state = HID_REQ_SET_IDLE;
			//UsbPrint("\r\n USBH_HID_ClassRequest HID_REQ_GET_REPORT_DESC -->HID_REQ_SET_IDLE" );
    }
  ...
  case HID_REQ_SET_IDLE:
    ...
    else if(classReqStatus == USBH_NOT_SUPPORTED) 
    {
      HID_Machine.ctl_state = HID_REQ_SET_PROTOCOL;        
		...
		//UsbPrint("\r\n USBH_HID_ClassRequest HID_REQ_SET_IDLE -->HID_REQ_SET_PROTOCOL" );
    break; 
  ...
  case HID_REQ_SET_PROTOCOL:
    ...
    if (USBH_Set_Protocol (pdev ,pphost, 0) == USBH_OK)
    {
      ...
      status = USBH_OK; 
			//UsbPrint("\r\n USBH_HID_ClassRequest HID_REQ_SET_PROTOCOL -->HID_REQ_IDLE" );
    } 
    break;
  ...
  //UsbPrint("\r\n USBH_HID_ClassRequest called (con=%d,stat=%d)",HID_Machine.ctl_state,status);
  return status; 
}
                                   ...
                                   void   *phost)
{
  ...
  case HID_IDLE:
		//UsbPrint("\r\n USBH_HID_Handle - HID_IDLE - call");
    HID_Machine.cb->Init();
    HID_Machine.state = HID_SYNC;
    //UsbPrint("\r\n USBH_HID_Handle - HID_IDLE - called");
  case HID_SYNC:
		//UsbPrint("\r\n USBH_HID_Handle - HID_SYNC - call");
