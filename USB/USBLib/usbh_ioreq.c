                             ...
                             uint16_t            length)
{
  ...
  case CMD_SEND:
    ...
    status = USBH_BUSY;
		...
		//UsbPrint("\r\n usbh_ioreq.USBH_CtlReq RequestState : CMD_SEND --> CMD_WAIT " );
    break;
  ...
  case CMD_WAIT:
     if (phost->Control.state == CTRL_COMPLETE ) 
    {
      ...
      status = USBH_OK;      
			//UsbPrint("\r\n usbh_ioreq.USBH_CtlReq RequestState : CMD_WAIT --> CMD_SEND,  Control.state =CTRL_IDLE " );
    }
    else if  (phost->Control.state == CTRL_ERROR)
    {
      ...
      phost->RequestState = CMD_SEND;
      //status = USBH_FAIL;
			status = USBH_OK; // <-- 바코드 스캐너는 여기를 통과 못해서 강제로 OK로 리턴
			//UsbPrint("\r\n usbh_ioreq.USBH_CtlReq RequestState : CMD_WAIT --> CMD_SEND,  Control.state =CTRL_ERROR " );
    }   
     else if  (phost->Control.state == CTRL_STALLED )
    {
      ...
      status = USBH_NOT_SUPPORTED;
			//UsbPrint("\r\n usbh_ioreq.USBH_CtlReq RequestState : CMD_WAIT --> CMD_SEND,  Control.state =CTRL_STALLED " );
    }
    break;
  ...
  default:
	...
	//UsbPrint("\r\n USBH_CtlReq RequestState=%d, stat = %d", phost->RequestState, status );
  return status;
}
